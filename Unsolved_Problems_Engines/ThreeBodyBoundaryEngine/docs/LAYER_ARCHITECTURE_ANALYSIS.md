...
⚠️ **명확화**: 본 레이어는 어떠한 정책, 행동, 해결책도 생성하지 않습니다.  
...
**현재 상태**: 개념 설계 완료 (구현 대기)

**역할**: "안전한 경로를 찾는" 탐색

**핵심 개념** (설계 완료):
- 안전 영역 탐색: `safe_region = {c | risk(c) < τ_safe}`
- 경로 위험도: `R_path = (1/n) × Σ risk(c_i)`
- 수렴 보장: `C_guarantee = ∀c ∈ path: stability(c) > θ_stable`
- 탐색 전략: 그리디, A* 유사, 확률적 탐색

**제약 조건**:
1. L2의 위험 지형을 절대 위반하지 않음
2. L0의 물리 법칙을 절대 위반하지 않음
3. 해결책을 "강제"하지 않음 (안전한 경로가 없으면 "없음" 반환)
...
**자세한 내용**: [개념 및 수식 참조 가이드](./CONCEPT_FORMULA_REFERENCE.md#l3-레이어-개념-설계)
